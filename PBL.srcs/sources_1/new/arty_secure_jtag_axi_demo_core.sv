...
//   - Return MUX: host 주소는 host_prdata/pready만 사용
//   - u_otp_host APB: psel == penable == host_hit_any (보드 안정)
//   - Loopback OTP device 포함 (USE_PMOD=0)
// ============================================================================
...
module arty_secure_jtag_axi_demo_core (
  ...
  // 디버그 SFR 포함: 0x00A0~0x00BC
  wire host_hit_dbg0 = in_sfr && (rb_addr >= 16'h00A0 && rb_addr <= 16'h00AC);
  wire host_hit_dbg1 = in_sfr && (rb_addr >= 16'h00B0 && rb_addr <= 16'h00BC); // ★ 버전/HS/DEV
  ...
  jtag_mailbox_regfile #(.AW(16)) u_regfile (
    .clk(pclk), .rst_n(presetn),
    .bus_cs   ((in_sfr && !host_hit_any) ? rb_cs : 1'b0),  // ★ host 주소는 제외
...
otp_client_apb #(.AW(16), .CMDW(2), .USE_PMOD(1'b1)) u_otp_host (
  ...
  wire host_sfr_sel = host_hit_any;  // 정확히 host 주소만 host 응답
